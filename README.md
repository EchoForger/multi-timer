...
番茄钟卡片显示今日专注次数和完整番茄钟数。点击番茄钟旁的统计图标，可在同一个菜单栏面板中查看每天的 24 小时专注时间线、开始/结束时刻、专注总时长和完成状态，并支持 CSV 导出与清空。运行中的番茄阶段会在重启后恢复。
...
- 每日 24 小时专注时间线、专注次数、完整番茄钟数、CSV 导出和统计清空
...
- 无账户、无遥测；默认仅使用本地数据
- 具备 Apple iCloud KVS entitlement 的签名构建可同步设置与番茄钟结束记录

普通状态保存在 `~/.config/multitimer/state.json`，番茄钟结束记录保存在 `~/.config/multitimer/pomodoro-stats.json`。每条记录只包含开始时间、结束时间和是否完整完成；每日统计均实时计算。当前 ad-hoc 公共构建没有 iCloud entitlement，会安全使用本地存储。
